package ru.lyrian.collections_operations_time.model.operations.operations;

import ru.lyrian.collections_operations_time.enumerations.ViewTag;
import ru.lyrian.collections_operations_time.util.Utility;

public class CopyOnWriteSearchByValue extends AbstractOperation {
    public CopyOnWriteSearchByValue(ViewTag viewTag) {
        super(viewTag);
    }

    @SuppressWarnings("ResultOfMethodCallIgnored")
    @Override
    protected void executeInternal() {
        Utility.copyOnWriteArrayList.get(Utility.copyOnWriteArrayList.size() / 2);
    }
}
